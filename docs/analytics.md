...
| `github_click` | Any GitHub link | `location`: `nav` \| `final-cta` \| `footer` \| `issues` \| `roadmap` \| `license` |
...
- **Per-FAQ-question expand** — too noisy for v0, low signal-to-action ratio
- **Hover / mouseover events** — almost always noise
- **Pageview duration as custom event** — Umami tracks this natively, no need to duplicate
